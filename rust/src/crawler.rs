                            ...
                            Some((url_str, depth)) => {
                                ...
                                match Url::parse(&url_str) {
                                    Ok(url) => {
                                        ...
                                        match Self::fetch_and_process_url(&client, &url, depth < max_depth).await {
                                            Ok(Some(doc)) => {
    ...
    async fn fetch_and_process_url(
        ...
        if text.trim().is_empty() {
            ...
            if extract_links {
                ...
                let _links: Vec<String> = fragment.select(&link_selector)
                    .filter_map(|link| {
                ...
                // This part would need to be implemented to add links back to the queue
                // We'd need to pass the url_queue and visited_urls to this function
            }            
